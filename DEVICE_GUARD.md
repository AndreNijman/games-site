...
Referrers are recorded only for top-level document navigations, and only when the referring host is outside `*.andrenijman.com`, so an internal click never overwrites the arrival. `Referrer-Policy: strict-origin-when-cross-origin` means a cross-site referrer arrives as a bare origin with no path. The console marks a row `crawler` when the user agent matches the same bot pattern the visit counters use, and shows the raw user agent, because browser/OS both read `Other` for any agent that is not a mainstream browser.
